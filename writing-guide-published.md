...
- `shortcode{ ... }`：渲染纯短代码内容。
- 普通围栏代码块：生成原生代码块。LeetCode 题解里的完整算法代码**优先使用围栏写法**，例如 ```` ```cpp ````。
- `js{ ... }`：也可以生成原生代码块，`js` 可以替换成 `python`、`cpp`、`bash`、`text` 等语言名；但在 LeetCode 题解中优先级低于普通围栏代码块。
- `html{ ... }`：插入自定义 HTML。
- `math{ ... }`：生成块级 KaTeX 公式。
...
- 如果 `code ...{}` 里面的代码本身会出现“单独一行的 `}`”，必须改用普通围栏代码块。
- 旧的 ` ```wp-render ` 仍然兼容，但新文章优先使用这里的函数式写法。

对当前站点的发布建议：

- 在 VS Code 里写作时，正文排版优先使用上面的函数式写法，不必再手写 ` ```wp-render `。
- LeetCode 题解中的完整算法代码优先使用普通围栏代码块，例如 ```` ```cpp ````，不要优先使用 `code cpp{}` 或 `cpp{}`。
...
题解的代码需要添加必要的注释，注意添加必要的知识解读与扩展知识部分
题解中的完整算法代码块必须优先使用普通 Markdown 围栏写法，例如 ```` ```cpp ````；不要优先使用 `code cpp{}` 或 `cpp{}`。只有短小片段或明确需要函数式转换时，才考虑函数式代码块。
...
### 普通代码块
LeetCode 题解、教程文章里的完整代码，优先使用兼容围栏写法：
```js
function hello() {
...
写算法题解时，尤其是 C++ 代码，优先级按下面顺序执行：

1. **首选普通围栏代码块**：例如 ```` ```cpp ````。这是题解代码的默认写法。
2. **只有短小代码片段或明确需要函数式转换时**，才使用 `code cpp{}` 或 `cpp{}`。
3. 如果代码里包含单独一行的 `}`，必须使用普通围栏代码块，避免和函数式块的结束符冲突。
...
主题代码高亮会自动处理 `pre > code`，并增加行号、复制、折行、全屏按钮。完整题解代码优先使用普通围栏代码块；短小代码片段可以使用 `code js{}`、`code python{}` 这种写法，不需要手写最终 HTML。只有在你明确要控制最终结构时，才使用 `html{}`。
