...
int main() {
    Solution solution;
    cout << "练习目标：25. Reverse Nodes in k-Group" << '\n';
    cout << "只需要补全 reverseKGroup() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        ListNode* list1 = buildList({1, 2, 3, 4, 5});
        ListNode* actual = solution.reverseKGroup(list1, 2);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(listToVector(actual));
        cout << '\n';
        cout << "预期结果：";
        printValue(vector<int>{2, 1, 4, 3, 5});
        cout << "\n\n";
    }
    return 0;
}
