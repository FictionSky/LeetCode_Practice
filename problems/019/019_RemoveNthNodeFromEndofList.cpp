...
int main() {
    Solution solution;
    cout << "练习目标：19. Remove Nth Node From End of List" << '\n';
    cout << "只需要补全 removeNthFromEnd() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        ListNode* list1 = buildList({1, 2, 3, 4, 5});
        ListNode* actual = solution.removeNthFromEnd(list1, 2);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(listToVector(actual));
        cout << '\n';
        cout << "预期结果：";
        printValue(vector<int>{1, 2, 3, 5});
        cout << "\n\n";
    }
    {
        ListNode* list1 = buildList({1});
        ListNode* actual = solution.removeNthFromEnd(list1, 1);
        cout << "示例 2" << '\n';
        cout << "当前结果：";
        printValue(listToVector(actual));
        cout << '\n';
        cout << "预期结果：";
        printValue(vector<int>{});
        cout << "\n\n";
    }
    return 0;
}
