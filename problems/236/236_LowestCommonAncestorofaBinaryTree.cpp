...
int main() {
    Solution solution;
    cout << "练习目标：236. Lowest Common Ancestor of a Binary Tree" << '\n';
    cout << "只需要补全 lowestCommonAncestor() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        TreeNode* root = buildTree({"3", "5", "1", "6", "2", "0", "8", "null", "null", "7", "4"});
        TreeNode* node1 = findNode(root, 5);
        TreeNode* node2 = findNode(root, 1);
        TreeNode* actual = solution.lowestCommonAncestor(root, node1, node2);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(actual == nullptr ? -1 : actual->val);
        cout << '\n';
        cout << "预期结果：";
        printValue(3);
        cout << "\n\n";
