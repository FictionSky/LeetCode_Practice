...
int main() {
    Solution solution;
    cout << "练习目标：114. Flatten Binary Tree to Linked List" << '\n';
    cout << "只需要补全 flatten() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        TreeNode* root = buildTree({"1", "2", "5", "3", "4", "null", "6"});
        solution.flatten(root);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(rightChainToVector(root));
        cout << '\n';
        cout << "预期结果：";
        printValue(vector<int>{1, 2, 3, 4, 5, 6});
        cout << "\n\n";
    }
    return 0;
}
