...
int main() {
    Solution solution;
    cout << "练习目标：572. Subtree of Another Tree" << '\n';
    cout << "只需要补全 isSubtree() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        TreeNode* tree1 = buildTree({"3", "4", "5", "1", "2"});
        TreeNode* tree2 = buildTree({"4", "1", "2"});
        auto actual = solution.isSubtree(tree1, tree2);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(actual);
        cout << '\n';
