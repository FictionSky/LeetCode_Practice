...
int main() {
    Solution solution;
    cout << "练习目标：92. Reverse Linked List II" << '\n';
    cout << "只需要补全 reverseBetween() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        ListNode* list1 = buildList({1, 2, 3, 4, 5});
        ListNode* actual = solution.reverseBetween(list1, 2, 4);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(listToVector(actual));
        cout << '\n';
        cout << "预期结果：";
        printValue(vector<int>{1, 4, 3, 2, 5});
        cout << "\n\n";
    }
    return 0;
}
