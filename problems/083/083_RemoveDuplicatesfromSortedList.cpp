...
int main() {
    Solution solution;
    cout << "练习目标：83. Remove Duplicates from Sorted List" << '\n';
    cout << "只需要补全 deleteDuplicates() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        ListNode* list1 = buildList({1, 1, 2, 3, 3});
        ListNode* actual = solution.deleteDuplicates(list1);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(listToVector(actual));
        cout << '\n';
        cout << "预期结果：";
        printValue(vector<int>{1, 2, 3});
        cout << "\n\n";
    }
    return 0;
}
