...
int main() {
    Solution solution;
    cout << "练习目标：876. Middle of the Linked List" << '\n';
    cout << "只需要补全 middleNode() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        ListNode* list1 = buildList({1, 2, 3, 4, 5});
        ListNode* actual = solution.middleNode(list1);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(listToVector(actual));
        cout << '\n';
        cout << "预期结果：";
        printValue(vector<int>{3, 4, 5});
        cout << "\n\n";
    }
    {
        ListNode* list1 = buildList({1, 2, 3, 4, 5, 6});
        ListNode* actual = solution.middleNode(list1);
        cout << "示例 2" << '\n';
        cout << "当前结果：";
        printValue(listToVector(actual));
