...
int main() {
    Solution solution;
    cout << "练习目标：100. Same Tree" << '\n';
    cout << "只需要补全 isSameTree() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        TreeNode* tree1 = buildTree({"1", "2", "3"});
        TreeNode* tree2 = buildTree({"1", "2", "3"});
        auto actual = solution.isSameTree(tree1, tree2);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(actual);
        cout << '\n';
    ...
    {
        TreeNode* tree1 = buildTree({"1", "2"});
        TreeNode* tree2 = buildTree({"1", "null", "2"});
        auto actual = solution.isSameTree(tree1, tree2);
        cout << "示例 2" << '\n';
        cout << "当前结果：";
        printValue(actual);
