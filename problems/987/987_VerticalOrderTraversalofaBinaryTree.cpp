...
int main() {
    Solution solution;
    cout << "练习目标：987. Vertical Order Traversal of a Binary Tree" << '\n';
    cout << "只需要补全 verticalTraversal() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        TreeNode* root = buildTree({"3", "9", "20", "null", "null", "15", "7"});
        auto actual = solution.verticalTraversal(root);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(actual);
        cout << '\n';
        cout << "预期结果：";
        printValue(vector<vector<int>>{{9}, {3, 15}, {20}, {7}});
        cout << "\n\n";
    }
    return 0;
}
