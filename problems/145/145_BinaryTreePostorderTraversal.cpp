...
int main() {
    Solution solution;
    cout << "练习目标：145. Binary Tree Postorder Traversal" << '\n';
    cout << "只需要补全 postorderTraversal() 的 TODO 区域，再重新运行本地样例。" << "\n\n";
    {
        TreeNode* root = buildTree({"1", "null", "2", "3"});
        auto actual = solution.postorderTraversal(root);
        cout << "示例 1" << '\n';
        cout << "当前结果：";
        printValue(actual);
        cout << '\n';
        cout << "预期结果：";
        printValue(vector<int>{3, 2, 1});
        cout << "\n\n";
    }
    return 0;
}
